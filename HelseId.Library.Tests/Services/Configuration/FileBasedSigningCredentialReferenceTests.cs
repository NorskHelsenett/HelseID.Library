...
[TestFixture]
public class FileBasedSigningCredentialReferenceTests
{
    ...
    [Test]
    public async Task GetSigningCredential_returns_valid_signing_credental()
    {
        string jwkFile = "";
        try
        {
            jwkFile = await SetupFileWithContent(ValidJwk);
            var signingCredentialReference = new FileBasedSigningCredentialReference(jwkFile);
            var signingCredential = await signingCredentialReference.GetSigningCredential();

            signingCredential.Algorithm.Should().Be("RS384");
        }
        finally
        {
            File.Delete(jwkFile);
    ...
    [Test]
    public async Task GetSigningCredential_throws_HelseIDException_when_file_contains_invalid_jwk()
    {
        string jwkFile = "";
        try
        {
            jwkFile = await SetupFileWithContent("not a jwk");

            var signingCredentialReference = new FileBasedSigningCredentialReference(jwkFile);
            Func<Task> getSigningCredential = signingCredentialReference.GetSigningCredential;

            var expectedMessage = $"The file {jwkFile} does not contain a valid Json Web Key";
            await getSigningCredential.Should()
                .ThrowAsync<HelseIdException>()
    ...
    private static async Task<string> SetupFileWithContent(string content)
    {
