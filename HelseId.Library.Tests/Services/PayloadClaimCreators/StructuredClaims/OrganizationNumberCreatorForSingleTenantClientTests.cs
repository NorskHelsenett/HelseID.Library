...
[TestFixture]
public class OrganizationNumberCreatorForSingleTenantClientTests
{
    ...
    private OrganizationNumberCreatorForSingleTenantClient _organizationNumberCreatorForSingleTenantClient = null!;
    ...
    public void Setup()
    {
        ...
        _organizationNumberCreatorForSingleTenantClient = new OrganizationNumberCreatorForSingleTenantClient();
    }

    [Test]
    public void CreateStructuredClaims_creates_structured_expected_claim()
    {
        var result = _organizationNumberCreatorForSingleTenantClient.CreateStructuredClaims(_payloadClaimParameters,
            out var structuredClaims);
        ...
        identifier!["system"].Should().Be("urn:oid:2.16.578.1.12.4.1.4.101");
        
        identifier!["type"].Should().NotBeNull();
        identifier!["type"].Should().Be("ENH");

        identifier!["value"].Should().NotBeNull();
        identifier!["value"].Should().Be(_payloadClaimParameters.ChildOrganizationNumber);
    }

    [Test]
    public void CreateStructuredClaims_throws_exception_when_missing_child_organization_number()
    {
        _payloadClaimParameters.ChildOrganizationNumber = "";

        Action shouldThrow = () => _organizationNumberCreatorForSingleTenantClient.CreateStructuredClaims(_payloadClaimParameters, out _);

        shouldThrow.Should().Throw<MissingChildOrganizationNumberException>();
    }

    [Test]
    public void CreateStructuredClaims_does_not_create_claims_when_UseOrganizationNumbers_is_false()
    {
